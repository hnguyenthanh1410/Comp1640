...
export class ContributionService {
	...
	public async createContribution(
		...
		if (files) {
			Object.keys(files).forEach(async (key) => {
				...
				fileUrl[key] = files[key];
				fileUrl[key].map((file) => this.mediaService.upload(file));
	
				await Promise.all(fileUrl[key])
			})
		}
	...
	public async updateContribution(
		...
		if (contribution.author.id === currentUser.id || currentUser.role.name === RoleName.MARKETING_COORDINATOR) {
			let data = {};
			const fileUrl = {
				files: undefined,
				photos: undefined
			...
			data = await this.contributionRepository.save({
				...contribution,
				name: payload.name,
			...
			if (payload.dueDate) {
				data = await this.contributionRepository.save({
					...
					period: {
						...contribution.period,
						closureDate: new Date(payload.dueDate).toDateString()
					}
				});
